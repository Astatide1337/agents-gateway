...
// credentials, if any, belong to the v3 loopback broker sidecar outside this process.
...
// ConfigFromEnv reads only adapter-owned environment variables. In particular,
// it never reads OPENAI_API_KEY, CODEX_HOME, or any other auth variable. In v3,
// AGW_BROKER names the in-pod loopback broker and the adapter derives its routes.
func ConfigFromEnv(getenv func(string) string) (Config, error) {
	...
	if brokerBase == "" {
		brokerBase = getenv("AGW_CODEX_BROKER_URL")
	}
	if brokerBase != "" {
		base, err := validateLoopbackBaseURL(brokerBase)
		if err != nil {
			return Config{}, fmt.Errorf("AGW_BROKER: %w", err)
		}
		if cfg.ResponsesURL == "" {
			cfg.ResponsesURL = base + ResponsesPath
		}
		if cfg.MCPURL == "" {
			cfg.MCPURL = base + MCPPath
		}
		if cfg.ArtifactURL == "" {
			cfg.ArtifactURL = base + ArtifactPath
		}
		if cfg.ArtifactCreateURL == "" {
			cfg.ArtifactCreateURL = base + ArtifactCreatePath
		}
	}
	if cfg.ResponsesURL == "" {
		return Config{}, errors.New("AGW_BROKER or AGW_CODEX_RESPONSES_URL is required")
	}
	var err error
	...
	if cfg.MCPURL != "" {
		if cfg.MCPURL, err = validateLoopbackEndpoint(cfg.MCPURL, MCPPath); err != nil {
			return Config{}, fmt.Errorf("AGW_CODEX_MCP_URL: %w", err)
		}
	}
	if cfg.ArtifactURL != "" {
		if cfg.ArtifactURL, err = validateLoopbackEndpoint(cfg.ArtifactURL, ArtifactPath); err != nil {
			return Config{}, fmt.Errorf("AGW_CODEX_ARTIFACT_URL: %w", err)
		}
	}
	if cfg.ArtifactCreateURL != "" {
		if cfg.ArtifactCreateURL, err = validateLoopbackEndpoint(cfg.ArtifactCreateURL, ArtifactCreatePath); err != nil {
			return Config{}, fmt.Errorf("AGW_CODEX_ARTIFACT_CREATE_URL: %w", err)
		}
	}
	...
	for name, destination := range map[string]*bool{
		"AGW_CODEX_ENABLE_TOOLS":     &cfg.EnableTools,
		"AGW_CODEX_REQUIRE_ARTIFACT": &cfg.RequireArtifact,
	} {
		value := getenv(name)
		if value == "" {
			continue
		}
		parsed, parseErr := parseBooleanEnv(value)
		if parseErr != nil {
...
func BuildArgs(cfg Config) ([]string, error) {
	...
	if cfg.EnableTools {
		if _, err := validateLoopbackEndpoint(cfg.MCPURL, MCPPath); err != nil {
...
func Run(ctx context.Context, input io.Reader, output io.Writer, diagnostics io.Writer, cfg Config) error {
	...
	if cfg.ResponsesURL == "" {
		return errors.New("loopback broker Responses endpoint is required")
	}
	...
	if processErr == nil && !codexOutput.sawTurnCompleted {
...
func uploadRunOutput(ctx context.Context, endpoint string, result map[string]any) (artifactcatalog.ArtifactRef, error) {
	if _, err := validateLoopbackEndpoint(endpoint, ArtifactPath); err != nil {
	...
	if err != nil {
		return artifactcatalog.ArtifactRef{}, errors.New("encode run output")
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodPut, endpoint, bytes.NewReader(payload))
	if err != nil {
		return artifactcatalog.ArtifactRef{}, errors.New("create run output request")
	}
	...
	if err != nil {
		return artifactcatalog.ArtifactRef{}, errors.New("upload run output")
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusCreated {
		_, _ = io.Copy(io.Discard, io.LimitReader(response.Body, 64<<10))
		return artifactcatalog.ArtifactRef{}, fmt.Errorf("run output upload returned HTTP %d", response.StatusCode)
	}
	decoder := json.NewDecoder(io.LimitReader(response.Body, 64<<10))
	decoder.DisallowUnknownFields()
	var artifact artifactcatalog.ArtifactRef
	if err := decoder.Decode(&artifact); err != nil {
		return artifactcatalog.ArtifactRef{}, errors.New("decode run output reference")
	}
	if err := artifact.Validate(); err != nil {
		return artifactcatalog.ArtifactRef{}, errors.New("invalid run output reference")
	}
	return artifact, nil
}
...
func consumeCodexOutput(reader io.Reader, e *emitter) outputResult {
	...
	for {
		...
		if len(line) != 0 {
			...
			case "turn.completed":
				if result.sawTurnCompleted {
					result.err = errors.New("Codex emitted multiple completed turns")
					return result
				}
