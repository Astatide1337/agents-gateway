...
func canonicalDecimal(value decimal) string {
	...
	// Positive exponents are integers. Emit ordinary integer notation so the
	// canonical document remains decodable into typed Go integer fields;
	// encoding/json correctly rejects an exponent token such as 6e1 for int32.
	// Negative exponents stay compact and mathematically canonical.
	if exponent > 0 {
		return prefix + value.digits + strings.Repeat("0", int(exponent))
	}
